...
namespace Xiyu.VirtualLiveRoom.Component.BarrageComments
{
    ...
    public class BulletCommentsScreenView : MonoBehaviour
    {
        ...
        {
            var handle = (AddressableGameObjectLoaderSo)await Resources.LoadAsync<AddressableGameObjectLoaderSo>("Settings/RefPrefabricate");
            var prefabricateAsset = await handle.LoadPrefabricateAssetAsync("Bullet Comment");


            _pool = new ObjectPool<BulletComments>(
                () =>
                {
                    var bulletComponent = Instantiate(prefabricateAsset, transform).GetComponent<BulletComments>();
                    bulletComponent.SetActive(false);
                    // bulletComponent.Registers(OnPointerEnter, OnPointerExit, OnPointerClick);
        ...
        public void SendBulletComment(string message)
        {
            ...
            for (var i = 0; i < _bulletComments.Count; i++)
            {
                ...
                var end = new Vector3(-windowSize.x, start.y, 0);
        ...
        private void OnPointerClick(BulletComments bulletComment)
        {
            ...
            var targetPosX = -windowSize.x.Half() - bulletComment.ContentRect.sizeDelta.x.Half() * scale.x;
            var targetPosY = -windowSize.y.Half() - bulletComment.ContentRect.sizeDelta.y.Half() * scale.y;
            ...
            bulletComment.ContentRect.DOScale(scale, 1.5F).OnComplete(() =>
            {
                ...
                inputField.onValueChanged.AddListener(content =>
                {
                    ...
                    bulletComment.ContentRect.anchoredPosition = new Vector2(
                        -windowSize.x.Half() - bulletComment.ContentRect.sizeDelta.x.Half() * scale.x,
                        -windowSize.y.Half() - bulletComment.ContentRect.sizeDelta.y.Half() * scale.y);
                });
